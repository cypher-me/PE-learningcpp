...
```cpp
    ...
    std::cout << "size of my_long_double:  " << sizeof(my_long_double) << std::endl; // to get size of variable
```

#### Fractional Numbers(floating Points/Types )

There are three types
| Type          | Size      | precision(no of digits)   |Comment |
| --------------| ----------| --------------------------| --- |
| float         | 4 bytes   |   7| |
| double        |8 bytes    | 15| reccomended |
| long double   |12 bytes   | >double| |

`std::setprecision(<value>)` controls the precision of std::cout
It is the number of digits that con be output by std::cout
You have to include `<iomanip>` for it to be present

##### floating points

A floting point variable ends with an ` f `
